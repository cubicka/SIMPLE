...
def selfplay_wrapper(env):
    class SelfPlayEnv(env):
        ...
        def setup_opponents(self):
            ...
            self.agent_player_num = np.random.choice(self.game.currentNPlayers)
            if self.opponent_type == 'mostly_best':
                self.agents = []
                for i in range(self.n_players):
                    j = random.uniform(0,1)
                    if j < 0.8:
                        opp = Agent('ppo_opponent', self.opponent_models[-1])
                    else:
                        start = 0
                        end = len(self.opponent_models) - 1
                        k = random.randint(start, end)
                        opp = Agent('ppo_opponent', self.opponent_models[k])
        ...
        def step(self, action):
            ...
            if not done and self.current_player_num != self.agent_player_num:
